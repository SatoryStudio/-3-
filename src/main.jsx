...
function buildOrderFinance(order, manufacturing) {
  ...
  const rows = [
    ...
    { id: 'print', group: 'Себестоимость', label: 'Печать: время станка', amount: manufacturing.printCost, direction: 'expense', status: 'confirmed', source: 'Время печати' },
  ...
  const pendingExpenses = rows
    .filter((row) => row.direction === 'expense' && row.status !== 'confirmed')
    .reduce((sum, row) => sum + row.amount, 0);
  const confirmedProfit = order.price - confirmedExpenses;
  const expectedProfit = order.price - expectedExpenses;
...
function getOrderView(order, catalogBySku) {
  ...
  const productName = product?.name || 'Нет в справочнике изделий';
...
function Money({ amount, direction = 'expense' }) {
  ...
  return <strong className={direction === 'income' ? 'money-income' : 'money-expense'}>{sign}{amount.toLocaleString('ru-RU')} ₽</strong>;
}
      ...
      {expandedFinance && (
        <section className="finance-analytics">
          <div className="cost-grid">
            <Metric label="Выручка" value={`${order.price} ₽`} />
            <Metric label="Расходы подтверждены" value={`${finance.confirmedExpenses.toLocaleString('ru-RU')} ₽`} />
            <Metric label="Ожидают отчета" value={`${finance.pendingExpenses.toLocaleString('ru-RU')} ₽`} />
            <Metric label="Расходы прогноз" value={`${finance.expectedExpenses.toLocaleString('ru-RU')} ₽`} />
            <Metric label="Прибыль факт" value={`${finance.confirmedProfit.toLocaleString('ru-RU')} ₽`} accent />
            <Metric label="Прибыль прогноз" value={`${finance.expectedProfit.toLocaleString('ru-RU')} ₽`} accent />
          </div>
          <div className="detail-card finance-card">
            ...
            <div className="finance-total">
              ...
              <Field label="Что еще ждем" value={`${finance.pendingExpenses.toLocaleString('ru-RU')} ₽`} />
            </div>
          </div>
        </section>
      )}
...
function Integrations({ productsSheetUrl, setProductsSheetUrl, catalogStatus, printerIntegration, setPrinterIntegration }) {
  ...
  function saveUsageSheet(event) {
    ...
    setUsageMessage('Таблица расхода сохранена. Следующий шаг — читать из нее историю печати и списания пластика.');
  }
  ...
  function saveMarketplace(event, item) {
    ...
    const next = {
      ...marketplaces,
      [item.id]: {
        ...marketplaces[item.id],
        connected: true,
        updatedAt: now,
        lastSync: marketplaces[item.id]?.lastSync || '',
      },
    };
    setMarketplaces(next);
    localStorage.setItem(MARKETPLACE_INTEGRATIONS_STORAGE_KEY, JSON.stringify(next));
    setMarketplaceMessage(`${item.title}: настройки сохранены. Теперь можно запускать синхронизацию заказов и фин. строк.`);
  }
  ...
  function syncMarketplace(item) {
    ...
    setMarketplaces(next);
    localStorage.setItem(MARKETPLACE_INTEGRATIONS_STORAGE_KEY, JSON.stringify(next));
    setMarketplaceMessage(`${item.title}: пробная синхронизация отмечена. В следующем шаге сюда подключается реальный API и запись строк в карточки заказов.`);
  }
    ...
    <section className="panel">
      ...
      <div className="integration-list">
        ...
        <article className="integration-card">
          <PlugZap size={22} />
          <div>
            <strong>Таблица расхода</strong>
            <span>История печати, личные работы, тесты, брак и фактическое списание пластика.</span>
          </div>
          ...
          {activeIntegration === 'usage' && (
            <form className="marketplace-settings" onSubmit={saveUsageSheet}>
              <label>
                <span>Таблица или лист расхода</span>
                <input value={draftUsageUrl} onChange={(event) => setDraftUsageUrl(event.target.value)} placeholder="https://docs.google.com/spreadsheets/d/..." />
              </label>
              <div className="format-hint">
                <strong>Что будет загружаться</strong>
                <span>Дата, источник расхода, SKU/заказ, тип пластика, цвет, граммы и статус печати.</span>
              </div>
              {usageMessage && <div className="catalog-status ready">{usageMessage}</div>}
              <div className="integration-actions">
                <button className="button primary" type="submit">Сохранить расход</button>
              </div>
            </form>
          )}
        </article>
        ...
        {items.map((item) => (
          <article className={marketplaces[item.id]?.connected ? 'integration-card connected' : 'integration-card'} key={item.id}>
            <PlugZap size={22} />
            <div>
              <strong>{item.title}</strong>
              <span>
                {marketplaces[item.id]?.connected
                  ? `Подключено локально · ${marketplaces[item.id].updatedAt}`
                : item.hint}
              </span>
            </div>
            ...
            {activeIntegration === `marketplace-${item.id}` && (
              <form className="marketplace-settings" onSubmit={(event) => saveMarketplace(event, item)}>
                ...
                <div className="printer-form-grid">
                  {item.fields.map((field) => (
                    <label key={field}>
                      <span>{field}</span>
                      <input
                        value={marketplaces[item.id]?.[field] || ''}
                ...
                {marketplaces[item.id]?.lastSync && (
                  <div className="catalog-status ready">Последняя синхронизация: {marketplaces[item.id].lastSync}</div>
                )}
                <div className="integration-actions">
                  <button className="button primary" type="submit">Сохранить настройки</button>
                  <button className="button ghost" type="button" onClick={() => syncMarketplace(item)} disabled={!marketplaces[item.id]?.connected}>
                    Синхронизировать
                  </button>
                  {marketplaces[item.id]?.connected && (
